...
% The map $y\mapsto\invm_{\!y}$ associates to each $\levs_y$ the measure induced from the equilibrium measure $\invm$.
...
% let us also consider the process $X^x_t$ conditioned to a fixed value~$y=\obs(x)\in\R^K$
% \begin{equation*}
%     X^x_t\cbar\levs_y = \Exp[][X^x_t\cbar \obs(X^x_t)=y],\quad t\in[0,T].
% \end{equation*}
...
% the process $Y_t$ stays largely unaffected by the fast fluctuations present in the dynamics of $X_t$, whereas this fluctuations are statistically bound to the value~$y$ of the slow observable.
...
% The multiscale nature of the process $X_t$ is encoded in the singular values of the diffusion matrix $\si$ or, equivalently, in the 
% % \note{connection with the spectrum of transition operator?}
% spectrum of the (variance-)covariance matrix of the process
% \begin{equation}\label{eq:var_cov}
%    \si(x) = \nu(x)\tp{\nu(x)}.
% \end{equation}
% In our case, the spectrum of $\si$ consists of (at least) two groups of eigenvalues separated by a large gap without any spectrum. The eigenvalues with large moduli are related to the fast modes of the dynamics and the size of the gap determines the time-scale separation.
...
\begin{enumerate}
    ...
    % \note{How to correctly describe that?}almost degenerate. 
    ...
    % If the process $X_t$ is initialized at some point $x$ and evolved over a short time window $\de t\sim\ep$, the resulting path will evolve close to the fiber.
...
\end{enumerate}
% \question{It is not necessary that $D^s + D^f = D$. Do we assume that for simplicity?}
...
% and a slow manifold~$\sman\subset\R^D$ which is a $D^s$-dimensional submanifold of $\R^D$ transversal to all the fast fibers.
...
\begin{center}
    ...
    % \item the Jacobian matrix $J_{\smap}$ is of full rank (i.e., $\rank J_\smap = D^s$)
\end{center}
...
% and to sample accurately the \emph{quasi-stationary} invariant distributions of
% fast variables, 
...
% We demonstrate the result of short bursting to sample the projection on the slow manifold and the fast fibers.
...
% In our case, the data comes naturally from the trajectories of the observed system, thus the scope of our data is the natural one for the systems we analyze. This explains why we still get good accuracy of the approximation by~$\enc$ of the slow variable~\eqref{eq:ohm_slow} on the test set (the leftmost panel) even though the level sets of the encoder are not globally accurate.

% \todo{Discuss complete case.}
...
% the slow variable is not well represented by the encoder due to poor matching of the encoder level sets with the actual fast fibers of the system.
\begin{figure}
    \centering
...
\end{figure}
% This misalignment, the more pronounced the further from the slow manifold we deviate, causes the encoder to mix data points from different fast fibers, which makes the structure of its level sets different from the fast fibers.
...
\begin{figure}
    ...
    % Looking at such statistics allows us to assess the performance of trained encoders: for slow views well approximating the slow variable, the gradients are almost perpendicular to all fast directions at every data point.
    }
...
% We explore this technique in Section~\ref{sec:essential_coords} to remove overfitting in too large networks and, more importantly, to cut off non-essential input nodes from the rest of the network.
